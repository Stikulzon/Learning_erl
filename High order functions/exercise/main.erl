...
get_stat(Champ) ->
    TeamStat = lists:map(fun get_stat_from_team/1, Champ),
    {TotalPlayers, TotalAge, TotalRating} = 
        lists:foldl(fun({Player, Age, Rating}, {TotalPlayers, TotalAge, TotalRating}) ->
                        {TotalPlayers + Player, TotalAge + Age, TotalRating + Rating} end, 
                    {0, 0, 0}, TeamStat),
    {length(Champ), TotalPlayers, TotalAge/TotalPlayers, TotalRating/TotalPlayers}.

get_stat_from_team({team, _TeamName, List}) ->
    lists:foldl(fun({player, _ChampName, Age, Rating, _HP}, {NumPlayers, TotalAge, TotalRating}) ->
                    {NumPlayers+1, TotalAge+Age, TotalRating+Rating}
                end,
            {0, 0, 0},
            List).
...
filter_sick_players(Champ) ->
    TeamSick = lists:map(fun filter_team_sick_players/1, Champ),
    lists:filter(fun({team, _, Players}) -> length(Players) >= 5 end, TeamSick).
    

filter_team_sick_players({team, _TeamName, List}) ->
    [{player, ChampName, Age, Rating, HP} || {player, ChampName, Age, Rating, HP} <- List, HP >= 50].
...
make_pairs(Team1, Team2) ->
    ...
    [{Player1, Player2} || {player, Player1, _, Elo1, _} <- Players1, {player, Player2, _, Elo2, _} <- Players2, Elo1 + Elo2 > 600].
